...
class VendorViewmodel(val repositary: TechorbitRepositary):ViewModel() {
    private val mutableLiveData= MutableLiveData<ResultOf<ModelVendror>>()
    val liveData : LiveData<ResultOf<ModelVendror>> =mutableLiveData
    fun getVendorReports(head:String,start:String,end:String)=viewModelScope.launch {
        mutableLiveData.value = repositary.getVendorReports(head,start,end)
    }
}